...
const AccountNav = () => {
  ...
  const subsubpage = pathname.split('/')?.[3];
  ...
  const linkClases = (type = null) => {
    ...
    if (type === subpage || (type === 'bookings/host' && subsubpage === 'host')) {
      classes += ' bg-primary text-white';
    } else if (type === 'bookings' && subpage === 'bookings' && subsubpage !== 'host') {
      classes += ' bg-primary text-white';
    } else {
      classes += ' bg-gray-200';
    }
  ...
  return (
    <nav className="mt-24 mb-8 flex w-full flex-col justify-center gap-2 p-8 md:flex-row md:p-0">
      ...
      <Link className={linkClases('bookings/host')} to={`/account/bookings/host`}>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
